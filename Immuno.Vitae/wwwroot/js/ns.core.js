...
String.prototype.format = function () { var _ = this; if (arguments.length > 0) { if (arguments.length == 1 && typeof arguments[0] == "object") { for (var i in arguments[0]) { _ = _.replace(new RegExp("({" + i + "})", "g"), arguments[0][i]); } } else { for (var i in arguments) { _ = _.replace(new RegExp("({)" + i + "(})", "g"), arguments[i]); } } } return _; }
String.prototype.realLength = function (length) {
    if (!length) {
        var l = this.length, charCode, rl = 0; for (var i = 0; i < l; i++) {
            charCode = this.charCodeAt(i);
            if (charCode >= 0 && charCode <= 128) { rl += 1; }
            else { rl += 2; }
        }
        return rl;
    } else {
        var l = this.length, charCode, current = l, rl = 0, bl = true; for (var i = 0; i < l; i++) {
            charCode = this.charCodeAt(i);
            if (charCode >= 0 && charCode <= 128) { rl += 1; if (rl > length && bl === true) { current = i; bl = false; } }
            else { rl += 2; if (rl > length && bl === true) { current = i; bl = false; } }
        }
        return { length: rl, current: current };
    }
}

String.prototype.omit = function (length) {
    var l = this.realLength(length); if (l.length < length) { return this; }
    return this.substr(0, l.current - 2) + "...";
}
    ...
    ns.layer = function (conf) {
        ...
        layer.render = function (fn) {
            ...
            if (layer.status != "simple") {
                context.find(".fa-window-minimize").on("click", function () {
                    layer.min();
                });
                context.find(".fa-window-maximize").show().on("click", function () {
                    layer.max();
                });
            }
            if (layer.icon) {
...
(function () {
    ...
    function datetimepicker(conf) {
        ...
        this.format = conf.format || "yyyy-MM-dd hh:mm:ss";
        this.override = conf.override || -1;
        this.background = conf.backround || "#886ed7";
        this.changed = conf.changed || function () { };
        this.startSun = conf.startSun == null ? false : conf.startSun;
        this.minDate = new Date();
        this.parent = conf.parent;
        this.zindex = conf.zindex || 99;
        var currentDate = new Date();
        this.headformat = conf.headformat || "hans";
        this.width = 210;
        this.dispatch = ns.dispatch("rendered", "rendering", "show", "hide", "valuechanged");

        this.height = 25;
        //
        this._simple_inputhtml = '<input type="textbox" readonly="true" style=" width:210px; height:25px; line-height:25px; cursor:default; background-color:#fafafa; height:20px; line-height:20px;"/>';

        this._container_html = '<div  style="z-index: 99; text-align: left; width: 210px; line-height: 19.2px; font-size: 16px; display: block; position: absolute; border-color: rgb(224, 224, 224); border-width: 0.1px; border-style: solid; background-color: white; left: 1045.08px; top: 144px; opacity: 1;"><style>.datetimepicker-pick{cursor:pointer;} .datetimepicker-title:hover{ background:#e0e0e0; } .datetimepicker-title{ font-size:14px;width:23px;height:23px;cursor:pointer;line-height:23px; text-align:center;border-radius:2px; margin-top:4px;margin-bottom:4px;font-weight:600;} .datepicker-left-arrow{ font-size:16px;width:23px;height:23px;cursor:pointer;line-height:20px; text-align:center;border-radius:2px;}.datepicker-left-arrow:hover{ background:#e0e0e0; } .datepicker-right-arrow:hover{ background:#e0e0e0; } .datepicker-right-arrow{ border-radius:2px;margin-right:3px;text-align:center;font-size:16px;width:23px;height:23px;cursor:pointer;line-height:20px; }.datetimepicker-old {color: #999999;}.datetimepicker-disabled {color: #999999;}.datetimepicker-new {color: #999999;}.datetimepicker {height: 30px;line-height: 30px;}.datetimepicker-pick:hover {color: black;background-color: #d4e5f5;}.datetimepicker-disabled:hover {color: black;background-color: #fafafa;color: #888888;}.datetimepicker-current {color: black;background-color: #d4e5f5;}</style>'
            + '<div class="datepicker-header" style=" font-weight:200; line-height:28px; text-align:center; width:100%; height:28px;"><div class="datepicker-left-arrow fa fa-angle-left" style=" float:left; margin-left:6px; margin-top:4px;margin-bottom:4px;"></div><div class="datetimepicker-title" style=" width:73%; height:23px;line-height:23px; float:left;"></div><div class="datepicker-right-arrow fa fa-angle-right" style=" float:left; margin-top:4px;margin-bottom:4px;"></div></div><table class="ns_datepicker" style=" margin-left:2%; margin-right:2%; width:96%;"></table></div > ';

        this.goPrev = function () {
            var date = new Date(this.year, this.month, this.day, this.hour, this.minute, this.second);
            if (this.currentType == "year") {
                this.categories.year.start -= 20;
                this.renderYears(this.categories.year.mesh, this.categories.year.start);
            }
            else if (this.currentType == "month") {
                this.year -= 1;
                this.renderMonth(this.categories.month.mesh);
            }
            else if (this.currentType == "day") {
                this.month -= 1;
                if (this.month <= 0) {
                    this.year -= 1;
                    this.month = 12;
                }
                this.renderDays(this.categories.day.mesh);
            }
                ...
                if (this.day <= 0) {
                    ...
                    this.day = this.month == 2 ? ((this.year % 400 === 0 || this.year % 4 === 0) ? 29 : 28) : this.monthdays[this.month];
                }
                this.renderHours(this.categories.hour.mesh);
            }
            else if (this.currentType == "minute") {
                this.hour -= 1;
                this.renderMins(this.categories.minute.mesh);
            }
            else if (this.currentType == "second") {
                this.minute -= 1;
                this.renderSecond(this.categories.second.mesh);
            }
            return this;
        };
        this.goNext = function () {
            if (this.currentType == "year") {
                ...
                this.day += 1;
                if (this.day > (this.month == 2 ? ((this.year % 400 === 0 || this.year % 4 === 0) ? 29 : 28) : this.monthdays[this.month])) {
                    this.month += 1;
                    if (this.month > 12) {
                        this.year += 1;
                        this.month = 1;
        ...
        this.renderYears = function (mesh, start, min, max) {
            ...
            for (var i = 0, j = -1; i < 20; i++) {
                ...
                showDates[j].push({ text: ns.fixNumberString(t, fixnumber), h: (t < min || t > max ? " datetimepicker-disabled  " : "") + (start == y ? " datetimepicker-current" : "") + " datetimepicker-pick", year: t });
            }
            var html = "<tbody>";
            ns.data(showDates).each(function (d) {
                html += "<tr>";
                ns.data(d).each(function (b) {
                    html += "<td style='text-align:center; height: 25px; line-height:25px;' datetimepicker-mode='year' options='" + JSON.stringify(b) + "' class='datetimepicker-pick " + ("ns_datepicker_day " + b.h + " " + (b.y == currentDate.getFullYear() ? " datetimepicker-current" : "")) + "'>" + b.text + "</td>";
                });
                html += "</tr>";
            });
            html += "</tbody>";
            this.container.find("table.ns_datepicker").html(html);
            event_click.call(this, null);
            return this;
        };
        function event_click() {
            var me = this;
            this.container.find(".datetimepicker-pick").on("click", function (e) {
                if (ns.select(this).hasClass("datetimepicker-disbled")) {
                    return;
                }
                var self = ns.select(this);
                me.clickCallback(self.attr("datetimepicker-mode"), JSON.parse(self.attr("options")), e);
            });
        }
        this.renderMonth = function (mesh, min, max) {
            ...
            for (var i = 0, j = 0; i < 12; i++) {
                ...
                showDates[j].push({ text: ns.fixNumberString(i + 1, fixnumber), h: (i < min || i > max ? " datetimepicker-disabled  " : "") + (hours == i ? " datetimepicker-current" : "") + " datetimepicker-pick", month: i + 1, year: y });
            }
            var html = "<tbody>";
            ns.data(showDates).each(function (d) {
                html += "<tr>";
                ns.data(d).each(function (b) {
                    html += "<td style='text-align:center; height: 25px; line-height:25px;' datetimepicker-mode='month' options='" + JSON.stringify(b) + "' class='datetimepicker-pick " + ("ns_datepicker_day " + b.h + " " + (b.y == currentDate.getFullYear() && (currentDate.getMonth() + 1) == b.m ? " datetimepicker-current" : "")) + "'>" + b.text + "</td>";
                });
                html += "</tr>";
            });
            html += "</tbody>";
            this.container.find("table.ns_datepicker").html(html);
            this.container.find(".datetimepicker-title").html(this.year);
            event_click.call(this, null);
            return this;
        };
        this.renderDays = function () {
            ...
            var day = month === 2 && (year % 400 === 0 || year % 4 === 0) ? 29 : (month === 2 ? 28 : datetimepicker.monthdays[month]);
            var month_1 = month - 1;
            preyear = month_1 < 1 ? preyear - 1 : preyear;
            month_1 = month_1 < 1 ? 12 : month_1;
            var predays = month_1 === 2 && (preyear % 400 === 0 || preyear % 4 === 0) ? 29 : (month_1 === 2 ? 28 : datetimepicker.monthdays[month_1]);
            if (week <= 0) {
                week = 7;
            }
            var j = 0;
            for (var i = 1; i <= week; i++) {
                showDates[0].push({ text: predays - week + i, month: month_1, h: "datetimepicker-old", day: predays - week + i, year: preyear });
            }
            for (var i = week, k = 1; i < day + week; i++ , k++) {
                ...
                showDates[j].push({ text: k, h: "datetimepicker-current-month", month: month, day: k, year: year });
            }
            for (var i = week + day, k = 1; i < 42; i++ , k++) {
                ...
                showDates[j].push({ text: k, h: "datetimepicker-new", month: month + 1, day: k, year: year });
            }


            var html = '<tr ><td  class="datepicker_dow" style= " text-align:center; color:#999999;" >一</td><td class="datepicker_dow" style=" text-align:center; color:#999999;">二</td><td class="datepicker_dow" style=" text-align:center; color:#999999;">三</td><td class="datepicker_dow" style=" text-align:center; color:#999999;">四</td><td class="datepicker_dow" style=" text-align:center; color:#999999;">五</td><td class="datepicker_dow" style=" text-align:center; color:#999999;">六</td><td class="datepicker_dow" style=" text-align:center; color:#999999;">日</td></tr>' + "<tbody>";
            ns.data(showDates).each(function (d) {
                html += "<tr>";
                ns.data(d).each(function (b) {
                    html += "<td style='text-align:center; height: 25px; line-height:25px;' datetimepicker-mode='day' options='" + JSON.stringify(b) + "' class='datetimepicker-pick " + ("ns_datepicker_day " + b.h + " " + (b.y == currentDate.getFullYear() && currentDate.getMonth() + 1 == b.m && currentDate.getDate() === b.d ? " datetimepicker-current" : "")) + "'>" + b.text + "</td>";
                });
                html += "</tr>";
            });
            html += "</tbody>";
            ...
            this.container.find("table.ns_datepicker").html(html);
            this.container.find(".datetimepicker-title").html(this.year + "&nbsp;" + this.month);
            event_click.call(this, null);
            return this;
        };
        this.renderHours = function (mesh, min, max, y, m, d) {
            ...
            for (var i = 0, j = -1; i < 24; i++) {
                ...
                showDates[j].push({ text: ns.fixNumberString(i, 2) + ":00", h: (i < min || i > max ? " datetimepicker-disabled  " : "") + (hours == i ? " datetimepicker-current" : "") + " datetimepicker-pick", hour: i, month: this.month, day: this.day, year: this.year });
            }
            ...
            ns.data(showDates).each(function (d) {
                html += "<tr>";
                ns.data(d).each(function (b) {
                    html += "<td style='text-align:center; height: 25px; line-height:25px;' datetimepicker-mode='hour' options='" + JSON.stringify(b) + "' class='datetimepicker-pick " + ("ns_datepicker_day " + b.h + " " + (b.y == currentDate.getFullYear() && currentDate.getMonth() == b.m && currentDate.getDate() === b.d ? " datetimepicker-current" : "")) + "'>" + b.text + "</td>";
                });
                html += "</tr>";
            });
            html += "</tbody>";
            ...
            this.container.find("table.ns_datepicker").html(html);
            this.container.find(".datetimepicker-title").html(this.year + "-" + this.month + "-" + this.day);
            event_click.call(this, null);
            return this;
        };
        this.renderMins = function (mesh, min, max, y, m, d, h) {
            ...
            for (var i = 0, j = -1; i < 60; i++) {
                ...
                showDates[j].push({ text: ns.fixNumberString(i, fixnumber), h: (i < min || i > max ? " datetimepicker-disabled  " : "") + (mins == i ? " datetimepicker-current" : "") + " datetimepicker-pick", minute: i, hour: this.hour, month: this.month, day: this.day, year: this.year });
            }
            ...
            ns.data(showDates).each(function (d) {
                html += "<tr>";
                ns.data(d).each(function (b) {
                    html += "<td style='text-align:center; height: 25px; line-height:25px;' datetimepicker-mode='minute' options='" + JSON.stringify(b) + "' class='datetimepicker-pick " + ("ns_datepicker_day " + b.h + " " + (b.y == currentDate.getFullYear() && currentDate.getMonth() == b.m && currentDate.getDate() === b.d ? " datetimepicker-current" : "")) + "'>" + b.text + "</td>";
                });
                html += "</tr>";
            });
            html += "</tbody>";
            ...
            this.container.find("table.ns_datepicker").html(html);
            this.container.find(".datetimepicker-title").html(this.year + "-" + this.month + "-" + this.day + "&nbsp;" + this.hour + ":00:00");
            event_click.call(this, null);
            return this;

        };
        this.renderSecond = function (mesh, min, max, y, m, d, h) {
            this.currentType = "second";
            var currentDate = new Date();
            var monthdays = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
            var showDates = [[]];
            var fixnumber = 1;
            ...
            for (var i = 0, j = -1; i < 60; i++) {
                ...
                showDates[j].push({ text: ns.fixNumberString(i, fixnumber), h: (i < min || i > max ? " datetimepicker-disabled  " : "") + (mins == i ? " datetimepicker-current" : "") + " datetimepicker-pick", second: i, minute: this.minute, hour: this.hour, month: this.month, day: this.day, year: this.year });
            }
            ...
            ns.data(showDates).each(function (d) {
                html += "<tr>";
                ns.data(d).each(function (b) {
                    html += "<td style='text-align:center; height: 25px; line-height:25px;' datetimepicker-mode='second' options='" + JSON.stringify(b) + "' class='datetimepicker-pick " + ("ns_datepicker_day " + b.h + " " + (b.y == currentDate.getFullYear() && currentDate.getMonth() == b.m && currentDate.getDate() === b.d ? " datetimepicker-current" : "")) + "'>" + b.text + "</td>";
                });
                html += "</tr>";
            });
            html += "</tbody>";
            ...
            this.container.find("table.ns_datepicker").html(html);
            this.container.find(".datetimepicker-title").html(this.year + "-" + this.month + "-" + this.day + "&nbsp;" + this.hour + ":" + this.minute + ":00");
            event_click.call(this, null);
        ...
        this._render = function () {
            ...
            var exec = datetimepicker_reg.exec(this.format);
            if (exec[1]) {
                if (exec[2] && /yy|yyyy/.test(exec[2])) {
                    this.hasYear = true;
                    this.categories.year = { mesh: exec[2], start: currentDate.getFullYear() - 10 };
                }
                if (exec[3] && /MM|MMM|M|MMMM/.test(exec[3])) {
                    this.hasMonth = true;
                    this.categories.month = { mesh: exec[3] };
                }
                if (exec[4] && /dd|d/.test(exec[4])) {
                    this.hasDay = true;
                    this.categories.day = { mesh: exec[4] };
                }
            }
            if (exec[5]) {
                if (exec[6] && /hh|h/.test(exec[6])) {
                    this.hasHour = true;
                    this.categories.hour = { mesh: exec[6] };
                }
                if (exec[7] && /mm|m/.test(exec[7])) {
                    this.hasMins = true;
                    this.categories.minute = { mesh: exec[7] };
                }
                if (exec[8] && /ss|s/.test(exec[8])) {
                    this.hasSecond = true;
                    this.categories.second = { mesh: exec[8] };
                }
            }
            if (this.hasYear) {
                this.renderYears(this.categories.year.mesh, this.categories.year.start);
                ...
                this.renderDays(this.categories.month.mesh);
            }
            else if (this.hasHour) {
                this.renderHours(this.categories.month.mesh);
            }
            else if (this.hasMins) {
                this.renderMins(this.categories.month.mesh);
            }
            return this;
        ...
        this.render = function () {
            if (this.host) {
                this.input = this.mode == "simple" ? ns.select(ns.parseHtml(this._simple_inputhtml)) : "";
                this.input.appendTo(this.host);
            }
            this.container = ns.select(ns.parseHtml(this._container_html));
            this.container.appendTo("body");
            var self = this;
            this.input.on("click", function () {
                self._render();
                self.show();
                var selector = self.input;
                var c = selector.getBoundingRect(), y = c.top + (ns.scrollTop() || 0), x = c.left + (ns.scrollLeft() || 0), w = c.width, h = c.height;
                var abottom = true;
                ...
                if (!abottom) {
                    //arrow.css({ "left": (selector.getBoundingWidth() / 2 - 10) + "px", "top": (context.object.getBoundingHeight() - 2) + "px", "border-color": fill + " transparent transparent transparent" });
                    self.container.css({ "left": (x + 8) + "px", "top": (y - self.input.getBoundingHeight() - 10) + "px", });
                }
                else {
                    //arrow.css({ "left": (selector.getBoundingWidth() / 2 - 10) + "px", "top": "-20px", "border-color": "transparent transparent " + fill + " transparent" });
                    self.container.css({ "left": (x + 8) + "px", "top": (y + h + 10) + "px", }); //"background-color": fill, "border-color": fill
                }
            });
            this.container.find(".datepicker-left-arrow").on("click", function () {
                self.goPrev();
            });
            this.container.find(".datepicker-right-arrow").on("click", function () {
                self.goNext();
            });
            this.container.find(".datetimepicker-title").on("click", function () {
                if (self.currentType == "year") { return; }
                else if (self.currentType == "month") { self.renderYears(); }
                else if (self.currentType == "day") { self.renderMonth(); }
                else if (self.currentType == "hour") { self.renderDays(); }
                else if (self.currentType == "minute") { self.renderHours(); }
                else if (self.currentType == "second") { self.renderMins(); }
            });
            return this;
        ...
        this.show = function () {
            ...
            this.dispatch.show.apply(this, null);
            return this;
        }
        this.hide = function () {
            this.container.hide();
            this.dispatch.hide.apply(this, null);
            return this;
        }
        this.host = null;
        ...
        this.date = null;
        this.categories = {};
        this.previousDatetimePicker = null;
        this.nextDatetimePicker = null;
        this.dateGap = null;
        this.clickCallback = function (type, value, e) {
            var end = false;
            if (type == "year") {
                this.year = value.year;
                console.log(this.year);
                if (this.hasMonth) {
                    this.renderMonth(this.categories.mesh);
                }
            }
            else if (type == "month") {
                this.year = value.year;
                ...
                if (this.hasDay) {
                    this.renderDays(this.categories.mesh);
                }
            }
            else if (type == "day") {
                this.year = value.year;
                ...
                this.day = value.day;
                if (this.hasHour) {
                    this.renderHours(this.categories.mesh);
                }
                else {
                    end = true;
                }
                ...
                if (this.hasMins) {
                    this.renderMins(this.categories.mesh);
                }
                else {
                    end = true;
                }
                ...
                if (this.hasSecond) {
                    this.renderSecond(this.categories.mesh);
                }
                else {
                    end = true;
                }
                ...
                this.day = value.day;
                this.hour = value.hour;
                this.mintue = value.mintue;
                this.second = value.second;
                end = true;
            }
            this.selectedDate = new Date(this.year, this.month - 1, this.day, this.hour, this.minute, this.second);
            if (end) {
                if (this.previousDatetimePicker != null && this.previousDatetimePicker.selectedDate > this.selectedDate) {
                    alert("不能选择比所关联的时间更小的时间");
                    return;
                }
                if (this.nextDatetimePicker != null && this.nextDatetimePicker.selectedDate < this.selectedDate) {
                    alert("不能选择比所关联的时间更大的时间");
                    return;
                }
                this.input.value(this.selectedDate.format(this.format));
                this.container.hide();
            }
            return this;
        };
        this.value = function (v) {
            if (v) {
                this.input.ns().value(v);
                this.selectedDate = new Date(v);
                return this;
            }
            return this.input.ns().value();
            ;
        };
        this.mesh = conf.mesh;
        ns.merge(this, conf);
    ...
    datetimepicker.headformats = {
        "short": ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"],
        "full": ["Sunday", "Monday", "Tuesday", "Wendnesday", "Thursday", "Friday", "Saturday", "Sunday"],
        "hans": ["日", "一", "二", "三", "四", "五", "六", "日"],
    };
    datetimepicker.monthdays = [0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
    ns.Datetimepicker = datetimepicker;
    ns.datetimepicker = function (conf) {
        var picker = new datetimepicker(conf);
        return picker;
    }
})();
//pager
(function () {
    ...
    function pager() {
        ...
        this.firstText = "首页";
        this.hasFirst = true;
        this.selectedCls = "pager-current";
        this.defaultCls = "pager-number";
        this.itemTemplate = "<span>{page}</span>";// "".replace("{page}", "1");=> <span>1</span>
        this.pageItems = [];
        this.mode = "center";
        this.isFirst = true;
        this.disabled = "pager-text-item";
        var style = "<style> .pager-number{  color:rgb(136,136,136);padding:0px;text-decoration:none;margin:3px;border-radius:3px;height:20px;width:20px;display:inline-block;text-align:center;line-height:20px;background-color:#eee;cursor:pointer;}.pager-number:hover { color:#fff;background-color:rgb(60,72,80)}.pager-text-item{ color:rgb(136,136,136);font-size:14px; padding:3px;padding-left:5px; padding-right:5px;text-decoration:none;margin:3px;border-radius:3px;height:20px;display:inline-block;text-align:center;line-height:20px;background-color:#eee;cursor:pointer;}.pager-text-item:hover { color:#fff;background-color:rgb(60,72,80)}.pager-current { height:20px;width:20px;line-height:20px;display:inline-block;text-align:center; color:#fff;background-color:#29c06d;margin:3px;border-radius:3px;} .pager-current:hover{ }</style > ";
        this.dispatch = ns.dispatch("pagechanged");
        this._init = function (conf) {
            this.dispatch.on("pagechanged", function (e, args) {
                this.current = args.number;
                this.render();
            });
            ns.merge(this, conf || {});
            if (this.host) {
                this.context = ns.select(this.host);
            }
        }
        this.goto = function (page) {
            if (page) {
                if (page > this.pageSize) { console.warn("转向页面大于本身页码的最大值"); return this;; }
                this.dispatch.pagechanged.apply(this, [null, { event: null, number: page, current: this.currentNo, eventTarget: null }]);
            } return this;
        }
        this._add = function (pageitem, no, txtbtn) {
            if (no < 1) { no = 1; }
            if (no > this.pageSize) { no = this.pageSize; }
            var that = this;

            ns.select(pageitem).addClass("pager-item");
            if (txtbtn == true) {//是否为文本类型的按钮
                if (no === this.current) {//如果是当前页，则禁用类
                    ns.select(pageitem).addClass(this.disabled);
                } else {//如果不是当前页，绑定事件
                    ns.select(pageitem).addClass("pager-text-item");
                    ns.select(pageitem).on("click", function (e, args) {
                        that.dispatch.pagechanged.apply(that, [e, { event: e || window.event, number: no, current: that.current, eventTarget: pageitem }]);
                    });
                }
                this.context.append(pageitem);
                return;
            }
            if (no === this.current) {
                ns.select(pageitem).addClass(this.selectedCls);
            } else {

                ns.select(pageitem).addClass(this.defaultCls);
                ns.select(pageitem).on("click", function (e, args) {
                    that.dispatch.pagechanged.apply(that, [e, { event: e || window.event, number: no, current: that.current, eventTarget: pageitem }]);
                });
            }
        ...
        this.render = function (t) {
            //html
            this.context.html(style);
            if (!this.pageSize) { return this;; }
            //this.isFirst = false;
            //if (!t && !this.isFirst) { return; }
            this.current = t ? t : this.current;
            this.current = this.current < 1 ? 1 : this.current;
            if (this.hasFirst) { this._add(ns.parseHtml(this.itemTemplate.format({ page: this.firstText })), 1, true); }
            if (this.hasPrev) {
                this._add(ns.parseHtml(this.itemTemplate.format({ page: this.prevText })), this.current - 1, true);
            }
            var beg = 0, ts = this.count; end = this.count;
            if (this.mode == "center") {
                ts = ts % 2 === 0 ? ts : ts - 1;
                beg = this.current - parseInt(ts / 2);
                end = beg + this.count - 1;
                if (beg < 1) { beg = 1; end = beg + this.count - 1; }
                if (end > this.pageSize) { end = this.pageSize; if (beg != 1) { beg = end - this.count + 1; } }
                if (beg < 1) {
                    beg = 1;
                }
            } else {
            ...
            for (; beg <= end; beg++) {
                this._add(ns.parseHtml(this.itemTemplate.format({ page: beg })), beg);
            }
            if (this.hasNext) { this._add(ns.parseHtml(this.itemTemplate.format({ page: this.nextText })), this.current + 1 > this.pageSize ? this.pageSize : this.current + 1, true); }
            if (this.hasLast) { this._add(ns.parseHtml(this.itemTemplate.format({ page: this.lastText })), this.pageSize, true); }
            return this;
        }
    ...
    ns.Pager = pager;
    ns.pager = function (conf) {
        var p = new pager(conf);
        return p;
    }
})();
...
(function (editor) {
    ...
    editor.prototype.viewmode = "split";
    editor.mode = "mini";
    var html = "<div class='editor-container' style= 'width:99%; height:140px; color:rgb(34,34,34); font-size:12px; padding-bottom:2px; border-width:0px; border-style:solid; border-color:#e0e0e0; margin:5px;' ><div class='edit-core' style=' font-size:12px; height:100%; width:100%; left:0px; width:100%; border-width:0px; border-style:solid; border-color:#e0e0e0; border-top-width:0px; margin-bottom:15px; box-shadow:gray 1px 1px #888888;; padding-top:5px;; padding-bottom:20px;; padding-left:3px;;'><div class='edit-header' style='font-size: 12px; left: 0px; width: 99%; border-width: 0.3px; border-style: solid; border-color: rgb(224, 224, 224); height: 24px; opacity: 0; display: none;'><ul class='icon-menu-left edit-icon-menu' style=' float:left; font-size:12px; margin:0px; padding:0px; margin-right:2px; list-style:none;'></ul></div>"
        + "<div class='edit-domain' style=' display:block; height:100%;width:100%;'><div class='editor-document' style=' display:block; height:100%;float:left;width:44%;padding:0px;border:0px;'><textarea class='text-domain' style=' width:98%; height:84%; overflow:auto; font-size:14px; padding:5px; border-width:1px; background-color:#fafafa;'></textarea></div><div class='editor-document-preview' style=' display:block; height:100%; width:44%;float:left;padding:0px;border:0px;'></div></div></div></div>";
    editor.prototype.renderToolbar = function () {
    }
    editor.prototype.execCMD = function () {
    }
    editor.execDocumentCMD = function () {
    }
    editor.prototype.dispatch = ns.dispatch("modechanged", "preview", "command", "toolhandler", "rendering", "rendered", "valuechanged", "keyevent");
    editor.prototype.previewdelay = 0;
    editor.prototype.valuechangeQueue = [];
    editor.prototype.language = "markdown";
    editor.language = {
        text: "text",
        html: "text/html",
        js: "text/javascript",
    ...
    editor.languageFunction = {
        text: function (src) {
            return src;
        },
        markdown: function (src) {
            return ns.marked(src);
        },
    }
    editor.parse = function (language, content) {
        if (language in editor.languageFunction) {
            return editor.languageFunction[language].call(this, content);
        }
    }
    editor.prototype.preview = function () {
        var html = editor.parse(this.language, this.value());
        this.container.find(".editor-document-preview").html(html);
    }
    editor.prototype.value = function (val) {
        if (!val) {
            return this.container.find(".editor-document textarea").value();
        } else {
            this.container.find(".editor-document textarea").value(val);
        }
    }
    editor.extends = {
        editorView: {
    ...
    editor.prototype.render = function () {
        this.dispatch.rendering.apply(this, this.mode);
        this.container = ns.select(ns.parseHtml(html));
        if (this.viewmode == "split") {
            this.container.find(".editor-document").css({ float: "left", "width": "44%" });
            this.container.find(".editor-document-preview").css({ float: "left", "width": "44%", display: "block" });
        } else {
            this.container.find(".editor-document").css({ float: "none", "width": "100%" });
            this.container.find(".editor-document-preview").css({ float: "none", "width": "100%", display: "none" });
        }
        if (this.host) {
            this.container.find(".editor-document-preview,.editor-document").css({ height: this.host.css("height") });
            this.container.appendTo(this.host);
        }
        this.renderToolbar();
        var self = this;
        this.container.find(".editor-document textarea").on("change", function (e) {
            self.dispatch.valuechanged.apply(self, [e]);
            if (self.viewmode == "split") {
                if (self.previewdelay == 0) {
                    self.preview();
                } else {
                    self.valuechangeQueue.push(e.key);
                }
            }
        }).on("keyup", function (e) {
            self.dispatch.keyevent.apply(self, [e]);
        });
        this.dispatch.modechanged.apply(this, null);

        this.dispatch.rendered.apply(this, this.mode);
    }
    ns.Editor = editor;
    ns.editor = function (conf) {
        //if (conf && conf.callback) {
        //    var callback = conf.callback;
        //    if (conf.callback) { delete conf.callback; }
        //    if (callback) {
        //        callback.apply(this, null);
        //    }
        //}
        var e = new editor(conf);
        return e;
    }
})(ns.editor);